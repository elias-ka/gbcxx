...
namespace cb
{
    ...
    void SdlWindow::draw(const FrameBuffer& buf)
    {
        ...
        int pitch = 0;
        if (SDL_LockTexture(m_screen_texture.get(), nullptr, reinterpret_cast<void**>(&pixel_buf),
                            &pitch))
        {
        ...
        {
            std::memcpy(pixel_buf, buf.data(), SCREEN_WIDTH * SCREEN_HEIGHT * sizeof(Rgba));
        }
        else
        {
        ...
        if (SDL_RenderCopy(m_renderer.get(), m_screen_texture.get(), nullptr, nullptr))
