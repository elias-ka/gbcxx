...
namespace frontend
{
        ...
        , m_height(height)
    {
        ...
        m_screen_texture.reset(SDL_CreateTexture(m_renderer.get(), SDL_PIXELFORMAT_ABGR8888,
    ...
    void sdl_window::draw(cb::video::frame_buffer& buf)
    {
        ...
        // does it even work?
        for (u32 y = 0; y < cb::video::screen_height; y++)
        {
            for (u32 x = 0; x < cb::video::screen_width; x++)
            {
                cb::color::rgba color = buf.pixel_color(x, y);
                // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                pixels[cb::video::screen_width * y + x] = static_cast<u32>(color);
            }
        }
