...
namespace cb
{
    ...
    {
        tick4();
        const auto lo = m_mmu.read(addr);
        tick4();
        const auto hi = m_mmu.read(addr + 1);
    ...
    {
        tick4();
        m_mmu.write(addr, static_cast<u8>(data));
        tick4();
        m_mmu.write(addr + 1, static_cast<u8>(data >> 8));
    }

    u8 Cpu::read_operand() { return cycle_read8(m_pc++); }
    ...
    void Cpu::push8(u8 value)
    {
        ...
        cycle_write8(reg(Reg16::sp), value);
    }
    ...
    u8 Cpu::pop8()
    {
        const auto value = cycle_read8(reg(Reg16::sp));
        set_reg(Reg16::sp, reg(Reg16::sp) + 1);
        return value;
    }
