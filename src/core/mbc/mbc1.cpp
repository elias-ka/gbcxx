...
auto Mbc1::read_rom(uint16_t address) const -> uint8_t
{
    const uint8_t bank
        = (address < 0x4000) ? (m_banking_mode == 0 ? 0 : (m_rom_bank & 0b0001'1111)) : m_rom_bank;
...
auto Mbc1::read_ram(uint16_t address) const -> uint8_t
{
    if (!m_ram_enabled) {
        return 0xFF;
    }
...
auto Mbc1::write_rom(uint16_t address, uint8_t value) -> void
{
    if (0x0000 <= address && address <= 0x1FFF) {
        ...
        const uint8_t new_rom_bank = ((m_rom_bank & 0x60) | lower_bits) % m_rom_banks_count;
        ...
        if (m_rom_banks_count > 32) {
            ...
            const auto new_rom_bank = static_cast<uint8_t>((m_rom_bank & 0x1F) | (upper_bits << 5));
            m_rom_bank = new_rom_bank;
        }
...
auto Mbc1::write_ram(uint16_t address, uint8_t value) -> void
{
    if (!m_ram_enabled) {
        return;
    }
