...
uint8_t Cpu::GetReg(R8 r) const
{
    switch (r)
    {
        ...
        case R8::F:
        {
            return static_cast<uint8_t>((zf_ << 7) | (nf_ << 6) | (hf_ << 5) | (cf_ << 4));
        }
    }
}
...
{
    const uint8_t lo = ReadOperand();
    const uint8_t hi = ReadOperand();
...
{
    const uint8_t hi = BusRead8(addr);
...
{
    BusWrite8(--sp_, (val >> 8) & 0xff);
    BusWrite8(--sp_, val & 0xff);
}

uint16_t Cpu::StackPop()
{
    const uint8_t lo = BusRead8(sp_++);
    const uint8_t hi = BusRead8(sp_++);
    return static_cast<uint16_t>((hi << 8) | lo);
}

void Cpu::HandleInterrupts()
{
    static const std::array<std::string_view, 5> kName = {
    ...
    const int int_bit = std::countr_zero(interrupts);
    assert(int_bit < 5);
    if_ &= ~(1 << int_bit);
    const auto loc = static_cast<uint16_t>(0x40 + (int_bit * 8));

    LOG_DEBUG("CPU: Handling interrupt {:X} ({})", loc, kName[size_t(int_bit)]);
...
{
    const uint8_t opcode = ReadOperand();
    LOG_TRACE("CPU: opcode {:X}", opcode);

    switch (opcode)
    {
        case 0xcb: InterpretPrefixedInstruction(); break;
        ...
        default:
        {
            LOG_ERROR("CPU: Unknown opcode {:X}", opcode);
        }
    }
}

void Cpu::InterpretPrefixedInstruction()
{
    const uint8_t opcode = ReadOperand();
    ...
    switch (opcode)
    {
        ...
        default:
        {
            LOG_ERROR("CPU: Unknown prefixed opcode {:X}", opcode);
        }
    }
}
