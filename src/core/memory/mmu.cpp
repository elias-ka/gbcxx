#include "core/memory/mmu.h"
#include "common/error.h"
#include "common/try.h"
#include <cstdint>

namespace core::memory
{
    const std::vector<uint8_t> default_bios = {
        };

    mmu::mmu(std::vector<uint8_t> bios)
        : m_bios(std::move(bios))
    {
    }

    tl::expected<uint8_t, common::error> mmu::read8(uint16_t addr) const
    {
        if (addr < 0x100 && m_in_bios)
            return m_bios[addr];

        return tl::make_unexpected(common::error::out_of_bounds_read);
    }

    tl::expected<uint16_t, common::error> mmu::read16(uint16_t addr) const
    {
        const auto lo = TRY(read8(addr));
        const auto hi = TRY(read8(addr + 1));
        return (hi << 8) | lo;
    }

    void mmu::write8(uint16_t addr, uint8_t data)
    {
        // todo
    }

    void mmu::write16(uint16_t addr, uint16_t data)
    {
        write8(addr, static_cast<uint8_t>(data));
        write8(addr + 1, static_cast<uint8_t>(data >> 8));
    }

} // namespace core::memory
